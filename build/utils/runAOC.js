...
const runAOC = (options) => {
    ...
    if (hasBasePath) {
        ...
        if (maybePath === undefined) {
            throw new Error(`Base path argument required after '${BASE_PATH_ARGUMENT}' command`);
        }
        basePath = maybePath;
    }
    let file;
    if (hasDataPath) {
        ...
        const maybePath = cliArgs[index];
        if (maybePath === undefined) {
            throw new Error(`Data file path argument required after '${DATA_PATH_ARGUMENT}' command`);
        }
        file = maybePath;
    }
    ...
    if (file === undefined) {
        ...
        const inputFile = `${useAltData ? "alt-" : ""}${isTest ? "test-" : ""}input.txt`;
        file = `${basePath}${year}/${day}/${inputFile}`;
    }
    // parse data to array
    const rawInput = fs_1.default.readFileSync(path_1.default.resolve(file), "utf8");
